...
cd('D:\MICCAI_BraTS_2019_Data_Training\HGG');
subs = dir;
subs = subs(3:end);
...
for sub = 1:length(subs)
    ...
    mkdir(fullfile(outdir,'HGG',subs(sub).name));
    mkdir(fullfile(outdir,'HGG',subs(sub).name,'1'));
    mkdir(fullfile(outdir,'HGG',subs(sub).name,'0'));
    for i = 1:size(img_noise,3)
        cur_slice = squeeze(img_noise(:,:,i));
        cur_seg = squeeze(seg(:,:,i));
        if sum( cur_seg(:) ) >= 1
            filename = fullfile(outdir,'HGG',subs(sub).name,'1',['slice',num2str(i),'_lesion.png']);
            imwrite(cur_slice,filename)
        else
            filename = fullfile(outdir,'HGG',subs(sub).name,'0',['slice',num2str(i),'_nonlesion.png']);
            imwrite(cur_slice,filename)
        end 
    end
